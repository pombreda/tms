...
void TestFrame::SetUpValues(ContraptionP contraption, 
  ...
  tc_surname_->ChangeValue(contraption_->Get<std::string>("phone"));
  tc_age_ = (wxTextCtrl*)FindWindowByName("ID_TEXTCTRL3");
  tc_age_->ChangeValue("");
  *tc_age_ << contraption_->Get<std::string>("email");
}

void TestFrame::OnSaveClick(wxCommandEvent& WXUNUSED(event)) {
  contraption_->Set<std::string>("name", tc_name_->GetValue().ToStdString());
  contraption_->Set<std::string>("phone", tc_surname_->GetValue().ToStdString());
  contraption_->Set<std::string>("email", tc_age_->GetValue().ToStdString());
  contraption_->Save();
